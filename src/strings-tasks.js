...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    const isLowercase = lowercaseLetters.includes(char);
    if (isUppercase) {
      const index = uppercaseLetters.indexOf(char);
      const encodedCharIndex = (index + 13) % 26;
      const encodedChar = uppercaseLetters[encodedCharIndex];
      encodedString += encodedChar;
    } else if (isLowercase) {
      const index = lowercaseLetters.indexOf(char);
      const encodedCharIndex = (index + 13) % 26;
      const encodedChar = lowercaseLetters[encodedCharIndex];
      encodedString += encodedChar;
    } else {
      encodedString += char;
    }
...
function getCardId(value) {
  ...
  const cardIndex = initialDeck.indexOf(value);
  return cardIndex;
}
